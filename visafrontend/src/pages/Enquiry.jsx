...
export default function Enquiry() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      else if (res.data.data && Array.isArray(res.data.data)) setUsers(res.data.data);
      else setUsers([]);
    } catch (err) {
          ...
          <tbody>
            {users.length > 0 ? (
              users.map((u, index) => (
                ...
                >
                  <td className="p-3 font-medium text-gray-700">{u.customerName}</td>
                  <td className="p-3 text-gray-600">{u.email || "-"}</td>
                  <td className="p-3 text-gray-600">{u.mobile || "-"}</td>
                  <td>
                    <span
